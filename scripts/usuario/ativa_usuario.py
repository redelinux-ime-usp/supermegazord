...
def ldap_disable(user):
	group = user.group
	return user.change_group('exaluno') and group.add_member(user)
	
to_run = ['mail', 'printer', 'nfs', 'ldap']
commands = {
	...
	'nfs':     lambda user: remote.run_script("nfs",     comando(user), "megazord") == 0,
	'ldap':    lambda user: ldap_disable(user)
}

status_conta = {}
for running in to_run:
	print("Desativando do {0}...\n".format(running))
